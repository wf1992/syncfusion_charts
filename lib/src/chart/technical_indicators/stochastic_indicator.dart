...
// ignore:unused_element
  void _initDataSource(StochasticIndicator<dynamic, dynamic> indicator) {
    ...
    _setSeriesRange(
        periodCollection, indicator, periodX, indicator._targetSeries[1]);
    if (indicator.showZones) {
      _setSeriesRange(
          upperCollection, indicator, xValues, indicator._targetSeries[2]);
      _setSeriesRange(
          lowerCollection, indicator, xValues, indicator._targetSeries[3]);
    }
  }
  ...
  List<dynamic> _stochasticCalculation(
      ...
      CartesianSeries<dynamic, dynamic> sourceSeries) {
      ...
      for (int i = 0; i < data.length; i++) {
        if (!(i < len)) {
          pointCollection.add(_getDataPoint(data[i].x, values[i - len], data[i],
              sourceSeries, pointCollection.length));
  ...
  List<CartesianChartPoint<dynamic>> _calculatePeriod(
    ...
    if (data.length > period) {
      ...
      for (int i = 0; i < period - 1; ++i) {
        maxs.add(0);
        mins.add(0);
        modifiedSource.add(_getDataPoint(
      ...
      for (int i = period - 1; i < data.length; ++i) {
        ...
        modifiedSource.add(_getDataPoint(data[i].x, (top / bottom) * 100,
            data[i], series, modifiedSource.length));
      }
    }
