...
// ignore:unused_element
  ...
  void _initDataSource(MacdIndicator<dynamic, dynamic> indicator) {
    ...
    CartesianSeries<dynamic, dynamic> histogramSeries, macdLineSeries;

    if (indicator.macdType == MacdType.histogram) {
      histogramSeries = indicator._targetSeries[1];
    } else {
      macdLineSeries = indicator._targetSeries[1];
      if (indicator.macdType == MacdType.both) {
        histogramSeries = indicator._targetSeries[2];
      }
    }
    ...
    _setSeriesRange(
        signalCollection, indicator, signalX, indicator._targetSeries[0]);
    if (histogramSeries != null) {
      _setSeriesRange(
          histogramCollection, indicator, histogramX, histogramSeries);
    }
    if (macdLineSeries != null) {
      _setSeriesRange(macdCollection, indicator, macdX, macdLineSeries);
    }
  }
