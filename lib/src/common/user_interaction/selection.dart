...
class SelectionSettings extends ChartSelectionBehavior {
  ...
  void selectRange() {
    bool isSelect = false;
    if (_selectionRenderer.series.selectionSettings.enable &&
        selectionController != null)
      selectionController.addListener(() {
        _selectionRenderer.chart._chartState.isRangeSelectionSlider = true;
        _selectionRenderer.selectedSegments.clear();
        _selectionRenderer.unselectedSegments.clear();
        final dynamic start = _selectionRenderer
            .series.selectionSettings.selectionController.start;
        final dynamic end =
            _selectionRenderer.series.selectionSettings.selectionController.end;
        for (int i = 0; i < _selectionRenderer.series._dataPoints.length; i++) {
          isSelect = start is DateTime
              ? (_selectionRenderer.series._dataPoints[i].xValue >=
                      start.millisecondsSinceEpoch &&
                  _selectionRenderer.series._dataPoints[i].xValue <=
                      end.millisecondsSinceEpoch)
              : (_selectionRenderer.series._dataPoints[i].xValue >= start &&
                  _selectionRenderer.series._dataPoints[i].xValue <= end);

          isSelect
              ? _selectionRenderer.selectedSegments
                  .add(_selectionRenderer.series.segments[i])
        ...
        ValueNotifier<int>(
            _selectionRenderer.chart._chartState.seriesRepaintNotifier.value++);
      });
    if (_selectionRenderer.chart._chartState.initialRender)
      _selectionRenderer.chart._chartState.isRangeSelectionSlider = false;
  }
